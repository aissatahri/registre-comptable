...
#define SourceJar "..\\target\\registre-comptable-1.3.6.jar"

; If a native launcher was built with Launch4j, include it and use it for shortcuts
#if FileExists('launcher-1.3.6.exe')
...
OutputBaseFilename=RegistreComptable-1.3.6-setup
OutputDir=Output
Compression=lzma
...
Source: "launcher-1.3.6.exe"; DestDir: "{app}"; Flags: ignoreversion
#endif
...
#if HAS_NATIVE_LAUNCHER
Name: "{group}\Registre Comptable"; Filename: "{app}\launcher-1.3.6.exe"; WorkingDir: "{app}"; IconFilename: "{app}\app.ico"
Name: "{commondesktop}\Registre Comptable"; Filename: "{app}\launcher-1.3.6.exe"; WorkingDir: "{app}"; IconFilename: "{app}\app.ico"; Tasks: desktopicon
#else
...
#if HAS_NATIVE_LAUNCHER
Filename: "{app}\launcher-1.3.6.exe"; WorkingDir: "{app}"; Flags: nowait postinstall skipifsilent shellexec
#else
